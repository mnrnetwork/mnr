...
8. **Verified mempool listing (open, post-Stage 0):** an explorer (explorer.xmr.club, running MoneroSpace) polls monerod's `/get_transaction_pool` for its live mempool feed. That endpoint is unrestricted-only in monerod, so no public node and therefore no relay can serve it; the explorer keeps its own node for that one call and takes everything else from mnr. mnr could serve the same shape itself: pool hashes from one upstream, the transactions through the hash-verified `/get_transactions` path, composed into a `get_transaction_pool`-shaped answer (`tx_blob`, `fee` and `blob_size` from the parsed transaction; `receive_time` honestly zero, as restricted nodes report it). Every entry would be proven to hash to its id, which no daemon offers, and an RPC-only explorer could drop its node entirely. Scope for Stage 1 (or a Stage 0 follow-up once the beta settles); needs a policy entry (composed, `Mnr-Verify: hash`, never cached), a per-token cost model (one light call per new pool transaction) and a note in `spec/verification.md`. Recorded 2026-09-06 so it is not forgotten.
